...
import net.voxton.voxtongen.plats.PlatOfficeBuilding;
import net.voxton.voxtongen.plats.PlatPark;
import net.voxton.voxtongen.plats.PlatUnfinishedBuilding;
        ...
        for (int x = 0; x < Width; x++) {
            for (int z = 0; z < Width; z++) {
                ...
                if (current == null) {
                    ...
                    /*
                     * for each plot randomly pick a plattype see if the
                     * "previous chunk" is the same type if so make the new
                     * plattype connected to the previous type if the new plot
                     * is shorter than the previous plot or if the new plot is
                     * shallower than the previous slot mark the previous plot
                     * to have stairs if plot does not have neighbors yet mark
                     * the plot to have stairs
                     */
